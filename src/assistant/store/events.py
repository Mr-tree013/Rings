...
Deduplication and transition atomicity are database properties here, not Python
bookkeeping: a partial unique index rejects reused `(source, external_id)` identities,
and every transition runs inside one `BEGIN IMMEDIATE` transaction that re-checks the
status it believed the event had.
"""
...
class SqliteEventRepository:
    ...
    def add(self, event: InboundEvent) -> InboundEvent:
        ...
        try:
            ...
            raise self._translate_integrity_error(event, exc) from exc
        return event

    def get(self, event_id: EventId) -> InboundEvent | None:
        row = self._database.connection.execute(
            f"SELECT {_COLUMNS} FROM inbound_events WHERE id = ?", (str(event_id),)
        ).fetchone()
        ...
        *,
        ...
        with self._database.transaction():
            ...
            if cursor.rowcount != 1:
                raise StoreError(
                    f"inbound event {event_id} changed while it was being updated"
                )
        return updated

    def _translate_integrity_error(
        self, event: InboundEvent, error: sqlite3.IntegrityError
